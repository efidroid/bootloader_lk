...
void *api_mmap_get_platform_mappings(void *pdata, lkapi_mmap_mappings_cb_t cb)
{
    ...
    pdata = cb(pdata, A53_SS_BASE, A53_SS_BASE, (A53_SS_END - A53_SS_BASE), LKAPI_MEMORY_DEVICE);
    pdata = cb(pdata, SYSTEM_IMEM_BASE, SYSTEM_IMEM_BASE, 1*MB, LKAPI_MEMORY_WRITE_THROUGH);
    pdata = cb(pdata, MSM_SHARED_BASE, MSM_SHARED_BASE, 1*MB, LKAPI_MEMORY_WRITE_THROUGH);
...
{
    pdata = cb(pdata, MSM_SHARED_BASE, 1*MB);
#ifdef RPMB_SND_RCV_BUF
